...
usage() {
    echo "Usage: ./perceive_no_license_macOS.sh [--runtime-path /path/to/MATLAB_Runtime_R2023a-or-newer]"
}

if [[ "$OSTYPE" != darwin* ]]; then
    echo "[perceive] This script is for macOS only. On Linux use perceive_no_license_linux.sh."
    exit 1
fi
...
log "Launcher started (macOS)"
[[ -n "$CUSTOM_RUNTIME_PATH" ]] && log "Custom runtime path requested: $CUSTOM_RUNTIME_PATH"

if [[ ! -e "$APP_PATH" ]]; then
    echo "[perceive] perceive.app not found:"
    echo "  $APP_PATH"
    echo "Build perceive for macOS, then try again."
    exit 1
fi
