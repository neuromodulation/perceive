        ...
        if isfield(js,datafields{b})
            ...
            switch datafields{b}
                case 'Impedance'
                    ...
                    if length(T.Properties.VariableNames) > 50
                        set(gcf,'position', [ 977    32   560   964])
                    end
                ...
                case 'BrainSenseTimeDomain'
                    ...
                    for c = 1:length(runs)
                        ...
                        if ~isempty(d.parrm) && popt.run_parrm_ecg
                            d.parrm_ecg=[];
                            d.parrm_ecg_cleaned=[];
                            for e = 1:size(raw,1)
                ...
                case 'LfpMontageTimeDomain'
                    ...
                    for c = 1:length(runs)
                        ...
                        d.hdr.label = d.label;
                        d.hdr.Fs = d.fsample;
                        d.fname = [hdr.fname '_run-LMTD' char(datetime(runs{c},'Inputformat','yyyy-MM-dd HH:mm:ss.sss','format','yyyyMMddhhmmsss'))];
                        alldata{length(alldata)+1} = d;
    ...
    for b = 1:length(alldata)
        ...
        if regexp(data.fname,'BSTD')
            ...
            try
                ...
                [x,x,bslfile] = perceive_ffind([bslfile(1:end-4) '*.mat']);
                if length(bslfile)==1
                    bslfile = bslfile{1};
                elseif length(bslfile)>1
                    odate = strsplit(fulldata.fname,'BrainSense');
                    odate = str2num(odate{2});
                        
                    for ff =1:length(bslfile)
                    bdate = strsplit(bslfile{ff},'BSL');
                    bdate = str2num(bdate{2}(1:end-4));
                    ddate(ff) = abs(odate-bdate);
                    end
                    [~,i]=min(ddate);
                    bslfile=bslfile{i};
                end    
