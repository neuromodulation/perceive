function ecg=perceive_ecgV2(data,fs,plotit)
%%
keep 
if ~exist('fs','var')
    fs = 250;
...
ns = length(data);
flipsignal=nanmedian(findpeaks(data,'SortStr','descend','NPeaks',round(ns/fs)))<nanmedian(findpeaks(-data,'SortStr','descend','NPeaks',round(ns/fs)));
if flipsignal
    zdata = zscore(-data);
else
    zdata = zscore(data);
...
[pks,iv,ws,proms]=findpeaks(zdata,'MaxPeakWidth',round(fs*0.15));
for a=1:length(pks)
    try
        sh(a) = pks(a)-nanmean(zdata([iv(a)-round(0.015*fs):iv(a)-1  iv(a)+1:iv(a)+round(0.015*fs)]));
    catch
        sh(a) = 0;
    end
end
[sh,is]=sort(sh,'descend');
pks=pks(is);
iv=iv(is);
ws=ws(is);
proms=proms(is);
i = 1:round(ns/fs);
ir = setdiff(1:ns,i);
hr = 60./nanmedian((diff(sort(iv([i])))./fs));
hrs = 60./nanstd((diff(sort(iv([i])))./fs));
nhrs=hrs/hr;
close all
figure
if hr > 55 && hr < 120
...
for a = ir   
    nhr = 60./nanmedian((diff(sort(iv([i ir(a)])))./fs));
    nnhrs = (60/nanstd((diff(sort(iv([i ir(a)])))./fs)))./nhr;
    ...
    if nnhrs < nhrs && ~ishr
            ...
            nhrs = nnhrs;
            ishr = nhr > 55 && nhr < 120;
    elseif ishr && nnhrs < nhrs && nhr > 55 && nhr < 120
        i=[i ir(a)];
        nhrs = nnhrs;
        hr = nhr;
    end
...
for a = 2:size(x,1)
    [r,l]=xcorr(nanmedian(ndata,1),x(a,:).*hamming(size(x,2))',fs);
    [mr,mi]=max(r);
    tlag = l(mi);
    if tlag >0
        n=n+1;
